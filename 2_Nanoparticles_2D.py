...
plt.figure()
sim.plot2D(output_plane=mp.Volume(center=mp.Vector3(0,0),size=mp.Vector3(2*(0.5*d+2*r+0.1*dpad),2*(r+0.1*dpad))),
...
plt.savefig("./sim_plots/2Nanoparticles2D_R{radius}nm_D{distance}.gif".format(radius=r*100,distance=d*100),dpi=150,bbox_inches='tight')

sim.use_output_directory()

#animate = mp.Animate2D(sim,
#                       fields=mp.Ez,
#                       output_plane=mp.Volume(center=mp.Vector3(0,0),size=mp.Vector3(2*(0.5*d+2*r+0.1*dpad),2*(r+0.1*dpad))),
#                       field_parameters={'alpha':0.8, 'cmap':'RdBu', 'interpolation':'none'},
#                       eps_parameters={'interpolation':'spline36', 'cmap':'binary','alpha':0.5, 'contour':True, 'contour_linewidth':1, 'frequency':None, 'resolution':None})

sim.run(#mp.to_appended("ez-slice", mp.output_efield_z), # need to get working
        mp.in_volume(mp.Volume(center=mp.Vector3(0,0),size=mp.Vector3(2*(0.5*d+2*r+0.1*dpad),2*(r+0.1*dpad))), mp.after_sources(mp.output_png(mp.Ez, "-Zc dkbluered"))),
        #mp.after_sources(animate),
...
if mp.am_master():    
    #animate.to_gif(fps=1E9, filename="./gifs/2Nanoparticles2D_R{radius}nm_D{distance}.gif".format(radius=r*100,distance=d*100))
    plt.figure(dpi=150)
    ...
    plt.tight_layout()
    plt.savefig("./scattering_efficiencies/mie_scattering_Au_R{radius}nm_D{distance}.png".format(radius=r*100,distance=d*100))
    np.savez("./npz/scatt_eff_res_R{radius}nm_D{distance}.npz".format(radius=r*100,distance=d*100),scatt_eff_meep=scatt_eff_meep)