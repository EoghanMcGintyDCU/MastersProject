...
geometry = GetShape(r,parse_args.shape,material)
...
dpad = 1.1*r
vol = mp.Volume(center=mp.Vector3(0,0), size=mp.Vector3(2*(r+dpad),2*(r+dpad)))
...
geometry_overlay = GetShape(r,parse_args.shape,mp.Medium(epsilon=5))
...
plt.figure()
plt.title("Enhancement {}nm".format(lm_wv))
plt.imshow(overlay_data, interpolation='spline36', cmap='binary')
plt.imshow(enhancement[:,:,43].transpose(), interpolation='spline36', cmap='plasma', alpha=0.9)
plt.axis('off')
plt.colorbar()
plt.savefig("{}/EnhancementVLumerical.png".format(save_folder),dpi=150,bbox_inches='tight')
plt.close()